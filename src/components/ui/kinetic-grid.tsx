...
export default function KineticGrid({
  ...
  children?: ReactNode;
  className?: string;
  globalColor?: "veriqas" | "default" | "monochrome";
}) {
  ...
  const draw = useCallback((now: number) => {
    ...
    const theme = {
      veriqas:    { bg: "#000000", lineActive: { r: 71, g: 204, b: 98, a: 0.9 }, nodeActive: { r: 71, g: 204, b: 98, a: 1.0 }, glow: "71,204,98",  ripple: "71,204,98" },
    ...
    for (let row = 0; row < rows; row++) {
      for (let col = 0; col < cols; col++) {
        const p = pts[row][col], pr = prox[row][col];
    ...
    for (const r of ripples) {
      ...
      ctx.beginPath(); ctx.arc(r.x, r.y, safeRadius, 0, Math.PI * 2);
  ...
  const animate = useCallback((now: number) => {
    const m = mouseRef.current, t = targetMouseRef.current;
