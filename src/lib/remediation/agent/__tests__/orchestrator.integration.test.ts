...
function mockAI(opts: { patchPath?: string; giveUp?: boolean; strategy?: string; patchContent?: string } = {}): AIClient {
  ...
  return {
    async completeJSON<T>(p: { stage: string }): Promise<AICompletion<T>> {
      switch (p.stage) {
        ...
        case "PLANNER": return c({ strategy: opts.strategy ?? "CRYPTOGRAPHIC_MIGRATION", why: "PQC", affectedFiles: ["src/auth/jwt.ts"], affectedDependencies: [], expectedSecurityImprovement: "PQC", expectedCompatibilityImpact: "low", verificationRequirements: ["rescan"] }) as AICompletion<T>;
        case "PATCHER": return c({ changes: [{ filePath: opts.patchPath ?? "src/auth/jwt.ts", changeType: "MODIFY", newContent: opts.patchContent ?? "// migrated\n", reason: "swap alg" }], notes: "" }) as AICompletion<T>;
...
before(async () => {
  ...
  const mk = (st: string) => db.cryptoObservation.create({ data: { ref: `O-${Date.now()}-${Math.random().toString(36).slice(2,6)}`, tenantId, sensorType: st, evidenceSource: "STATIC_DETECTION" as never, observedAt: new Date(), algorithm: "RSA-2048", primitiveType: "DIGITAL_SIGNATURE" as never, purpose: "JWT signing", filePath: "src/auth/jwt.ts", quantumClass: "QUANTUM_VULNERABLE" as never, confidence: 90, scanJobId: sj.id } as never, select: { id: true } });
...
test("policy is computed, recorded and versioned on every attempt", async () => {
  await db.verificationRun.deleteMany({ where: { caseId } });
  await runRemediation(caseId, tenantId, { ai: mockAI(), env: fakeEnv([]) });
  const a = await db.remediationAttempt.findFirst({ where: { caseId }, orderBy: { createdAt: "desc" },
    select: { strategyPolicyVersion: true, policyJson: true } });
  assert.ok(a?.strategyPolicyVersion, "attempt records the policy version it was decided under");
  const pol = a!.policyJson as unknown as { permittedStrategies: string[]; prohibitedTargets: string[]; inputDigest: string };
  assert.ok(pol.permittedStrategies.includes("CRYPTOGRAPHIC_MIGRATION"));
  assert.ok(!pol.permittedStrategies.includes("CODE_CHANGE"), "quantum-vulnerable signature cannot be a code change");
  assert.ok(pol.prohibitedTargets.some(t => /ed25519/i.test(t)), "Ed25519 banned as a replacement");
  assert.ok(pol.inputDigest);
});

test("planner selecting a prohibited strategy is rejected as OUT_OF_POLICY", async () => {
  // CODE_CHANGE is not permitted for a quantum-vulnerable signature.
  const out = await runRemediation(caseId, tenantId, { ai: mockAI({ strategy: "CODE_CHANGE" }), env: fakeEnv([]) });
  assert.equal(out.finalStatus, "OUT_OF_POLICY");
  const a = await db.remediationAttempt.findFirst({ where: { caseId }, orderBy: { createdAt: "desc" }, select: { status: true, error: true } });
  assert.equal(a?.status, "OUT_OF_POLICY");
  assert.match(a?.error ?? "", /does not permit/i);
});

test("a permitted strategy whose PATCH introduces a prohibited primitive is rejected before verification", async () => {
  // The loophole: declare an allowed strategy, then write Ed25519 anyway.
  const before = await db.verificationRun.count({ where: { caseId } });
  const out = await runRemediation(caseId, tenantId, {
    ai: mockAI({ strategy: "CRYPTOGRAPHIC_MIGRATION", patchContent: "import { ed25519 } from '@noble/curves/ed25519';\nexport const sign = (m) => ed25519.sign(m, sk);\n" }),
    env: fakeEnv([]),
  });
  assert.equal(out.finalStatus, "OUT_OF_POLICY");
  const a = await db.remediationAttempt.findFirst({ where: { caseId }, orderBy: { createdAt: "desc" }, select: { status: true, error: true } });
  assert.equal(a?.status, "OUT_OF_POLICY");
  assert.match(a?.error ?? "", /introduces prohibited cryptography/i);
  assert.equal(await db.verificationRun.count({ where: { caseId } }), before, "must be rejected BEFORE any verification runs");
});

test("a compliant post-quantum patch is not blocked by the policy", async () => {
  const out = await runRemediation(caseId, tenantId, {
    ai: mockAI({ strategy: "CRYPTOGRAPHIC_MIGRATION", patchContent: "import { ml_dsa65 } from '@noble/post-quantum/ml-dsa';\nexport const sign = (m) => ml_dsa65.sign(sk, m);\n" }),
    env: fakeEnv([]),
  });
  assert.notEqual(out.finalStatus, "OUT_OF_POLICY", "a genuine PQC migration must pass the policy gate");
});
